...
class TextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512, evaluate=False):
        if evaluate and args.eval_data_file is None:
            # need to split the training file
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            valid_split = int(len(text) * (args.validation_split_percentage / 100))
            
            if evaluate and args.eval_data_file is None:
                print("split the training file for evaluation")
                self.k = random.randint(0, (100//args.validation_split_percentage) - 1)
                # this is for evaluation
                text = text[self.k*valid_split: (self.k+1)*valid_split]
            if not evaluate and (args.eval_data_file is None and args.do_eval):
                # this is for training
                text = text
                # text = text[:self.k*valid_split] + text[(self.k+1)*valid_split:]
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if args.line_by_line:
        return LineByLineTextDataset(tokenizer, args, file_path=file_path, block_size=args.block_size, evaluate=evaluate)
    else:
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    if args.inca:
        compression = hvd.Compression.inca(params={'nclients': 1, 'd': pytorch_total_params_trainable, \
        # compression = bps.Compression.inca(params={'nclients': 2, 'd': pytorch_total_params_trainable, \
        ...
        compression = hvd.Compression.dummy(params={'modify_idx': -1})
    elif args.topk:
        compression = hvd.Compression.topk(params={'d': pytorch_total_params_trainable, \
            'ef': args.ef, 'kp': args.kp})
    else:
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    # record evaluation loss and perplexity
    lr_and_loss.append(eval_loss)
    lr_and_loss.append(perplexity)   
...
def main():
    ...
    # Evaluation
    results = {}
    if args.do_eval and (args.local_rank == -1 or hvd.rank() == 0):
    ...
    if args.save_lr_loss:
        ...
        with open('../example/pytorch/accuracy_and_lr_record/wikitext-2-raw/'+args.model_type+'-'+curr_time+'.pkl', 'wb') as pkl_file:
