...
# Minghao: get link plags for static DPDK
def get_dpdk_link_flags():
    static_output = subprocess.check_output(['pkg-config', '--static', '--libs', 'libdpdk'])\
        .decode('utf-8').rstrip('\n')
    return shlex.split(static_output)

def get_dpdk_cpp_flags():
    dpdk_cflags = subprocess.check_output(['pkg-config', '--cflags', 'libdpdk'])\
        .decode('utf-8').rstrip('\n')
    return shlex.split(dpdk_cflags) + ['-DALLOW_EXPERIMENTAL_API']
...
def get_cpp_flags(build_ext):
    last_err = None
    default_flags = ['-g', '-std=c++11', '-fPIC', '-Ofast', '-Wall', '-fopenmp', '-march=native', '-mno-avx512f']
    flags_to_try = []
    # Minghao: add c flags for dpdk
    default_flags += get_dpdk_cpp_flags()
...
def get_link_flags(build_ext):
    ...
    # Minghao: add LDFLAGS for dpdk
    ld_flags += get_dpdk_link_flags()
    flags_to_try = []
    if sys.platform == 'darwin':
...
def get_common_options(build_ext):
    ...
    SOURCES = ['byteps/common/common.cc',
               ...
               ######## Minghao
               'byteps/common/dpdk/client/dpdk_manager.cc',
               'byteps/common/compressor/impl/drive.cc']
               ########
...
def build_server(build_ext, options):
    ...
    # auto-detect rdma
    if has_rdma_header():
        options['LIBRARIES'] += ['rdmacm', 'ibverbs', 'rt']
    else:
        options['LIBRARIES'] += []
    if use_ucx():
        options['LIBRARIES'] += ['ucp', 'uct', 'ucs', 'ucm']
        ucx_home = get_ucx_home()
        if ucx_home:
            options['INCLUDES'] += [f'{ucx_home}/include']
            options['LIBRARY_DIRS'] += [f'{ucx_home}/lib']

    ext = TorchExtension(server_lib.name,
                         define_macros=updated_macros,
                         include_dirs=options['INCLUDES'],
                         sources=['byteps/server/server.cc',
                          'byteps/common/cpu_reducer.cc',
                          'byteps/common/logging.cc',
                          ...
                          'byteps/common/compressor/impl/drive.cc'],
                        #  extra_compile_args=pytorch_compile_flags,
                         extra_compile_args=pytorch_compile_flags + ['-DBYTEPS_BUILDING_SERVER'],
                         extra_link_args=options['LINK_FLAGS'],
                         extra_objects=options['EXTRA_OBJECTS'],
                         library_dirs=options['LIBRARY_DIRS'],
                         libraries=options['LIBRARIES'])

    # Patch an existing server_lib extension object.
    for k, v in ext.__dict__.items():
        server_lib.__dict__[k] = v
    build_ext.build_extension(server_lib)

def build_cpu_server(build_ext, options):
