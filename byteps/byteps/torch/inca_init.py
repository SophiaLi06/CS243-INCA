...
class _DistributedOptimizer(torch.optim.Optimizer):
    def __init__(self, params, named_parameters, compression,
        ...
        if isinstance(self._compression, Compression.inca):
        ...
        self.max_info_tensor = torch.zeros(self.num_workers).cuda()
        # declare tensors
    ...
    def _push_pull_info(self, param_handles):
        ...
        """
        #TODO: for now, just directly set the field
    ...
    def _push_pull_max_norm(self, max_norm):
        ...
        handle = byteps_push_pull(self.max_info_tensor, average=False, name="Gradient.max_info")
        return handle
    ...
    def _make_hook(self, p):
        def hook(*ignore):
            ...
            # print("insert handle for " + tensor_name + " in hook")
    ...
    def synchronize(self):
        ...
        for p in missing_p:
            if self._is_tensor_instance:
                ...
                tensor_name = self._parameter_names.get(p)
            # tensor_norm = p.grad.norm(2).item()
            # print("insert handle for " + tensor_name + " in synchronize because of missing_p")
            if self._last_insert_tensor:
                # update the "next" record for the last inserted tensor
                self._name_to_next[self._last_insert_tensor] = tensor_name
            # self._name_to_norm[tensor_name] = tensor_norm
            self._last_insert_tensor = tensor_name
            handle, ctx = None, None
            # handle, ctx = self._push_pull_grad_async(p)
            self._handles[p] = (handle, ctx)
        ...
        if isinstance(self._compression, Compression.newinca) or isinstance(self._compression, Compression.inca)\
             or isinstance(self._compression, Compression.topk):
            ...
            # NOTE: disable the following line (230-233) for running without norm communication
            # communicate the norms first
            self.batch_push_pull_info(grads)
            # NOTE: use _push_pull_info for compressing each layer individually
            # for tensor_name in param_names:
            #     global_max_norm = send_norm("Info."+tensor_name, self._compression_info['max_norm'])
            #     # print("global_max_norm: ", global_max_norm)
            #     if global_max_norm:
            #         self._compression_info['max_norm'] = global_max_norm

            #     print("Gradient."+tensor_name, str(self._compression_info['max_norm']))
