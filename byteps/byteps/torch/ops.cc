...
#ifdef USE_INCA
  return BytePSGlobal::GetDPDK()->CommunicateNorm(context.declared_key, max_norm);
#else
...
float GetNorm(const std::string& name){
  std::string tensor_name = GetOpName("byteps", name.c_str(), 0);
  auto& context = common::GetContextFromName(tensor_name);
  if (context.initialized){
    // printf("context norm for %s after EnqueueTensor: %.6f\n", tensor_name.c_str(), context.norm);
    return context.norm;
  }
  else return 0.0;
...
void StartTask(::torch::Tensor tensor, ::torch::Tensor output, int average,
  ...
  common::InitTensor(context, size, dtype,
                      ...
                      : nullptr);
  // printf("Inited tensor %s with compressor %s\n", tensor_name.c_str(), context.compressor_name.c_str());
  ...
  while(accumulated < size){
    dpdk_key = BytePSGlobal::GetDPDKKey();
    accumulated += bound;
  }
  // printf("Tensor %s has dpdk_key %" PRIu64 "\n", tensor_name.c_str(), dpdk_key);
#endif
...
#ifdef USE_INCA
    // StartTask(tensor, output, average, tensor_name, version, priority, handle, norm);
