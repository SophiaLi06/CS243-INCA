  ...
  while (true) {
    ...
    switch (msg.ops) {
      case COPY_FIRST: {
        ...
        //   // printf("COPY_FIRST UINT8 tensor of length %d\n", msg.len);
        //   for (size_t i = 0; i < msg.len; i++) {
        //     ((uint8_t*)msg.src)[i] = recv_table[((uint8_t*)msg.src)[i]];
        //   }
        // }
      ...
      case SUM_RECV: {
        ...
        //   // printf("SUM_RECV UINT8 tensor of length %d\n", msg.len);
        //   for (size_t i = 0; i < msg.len; i++) {
        //     ((uint8_t*)msg.src)[i] = recv_table[((uint8_t*)msg.src)[i]];
        //   }
        // }
...
void BytePSHandler(const ps::KVMeta& req_meta,
  ...
  // process TopK packet
  if (type.requestType == RequestType::kTopKPushPull){
    printf("recieved kTopKPushPull request\n");
  }
