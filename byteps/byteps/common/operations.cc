...
void byteps_lazy_init() {
  BytePSGlobal::Init();
#ifdef USE_INCA
  /* add dpdk listener thread */
  BytePSGlobal::DPDKListener = new std::thread(DPDKListenerLoop);
  BytePSGlobal::DPDKReset = new std::thread(DPDKResetLoop);
#endif
#ifdef USE_P4ML
...
Status EnqueueTensor(BPSContext &context, std::shared_ptr<Tensor> input,
  ...
  for (size_t i = 0; i < partitions.size(); ++i) {
    ...
    ////////////////
#ifdef USE_INCA
    task->dpdk_key = (dpdk_key - (partitions.size() - i - 1)) % DPDK_KEY_TOTAL;
    BytePSGlobal::DPDKTask[task->dpdk_key] = task;
#endif
#ifdef USE_P4ML
  ...
  //                << ", rank=" << BytePSGlobal::GetLocalRank();
  // below is the original log level
  BPS_LOG(TRACE) << "EnqueueTensor finished: " << name
                 << ", rank=" << BytePSGlobal::GetLocalRank();
  ////////////
  // printf("Enqueued finish for tensor %s", name.c_str());
  return Status::OK();
}
...
void InitTensor(BPSContext &context, size_t size, int dtype, void *cpubuff) {
  ...
  if (BytePSGlobal::IsCrossPcieSwitch()) {
    ...
    context.cpubuff = shm_obj->openSharedMemory(std::string("BytePS_shared_Mem_"),
