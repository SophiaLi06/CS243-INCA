...
class ReadyTable {
 public:
  ...
  // to access or modify the per_worker scale and norm
  float GetKeyScale(uint64_t key);
  void SetKeyScale(uint64_t key, float scale);
  float GetKeyNorm(uint64_t key);
  void SetKeyNorm(uint64_t key, float max_norm);
 ...
 private:
  ...
  // (key, norm) pair, keeps track of the per-worker borm
  std::unordered_map<uint64_t, float> _norm_table;
};
