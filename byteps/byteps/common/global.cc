...
// uint32_t BytePSGlobal::_partition_bytes = sizeof(float) * MAX_ENTRIES_PER_PACKET * 4;
#else
uint32_t BytePSGlobal::_partition_bytes = 4096000;
#endif
...
#ifdef USE_INCA
int BytePSGlobal::_use_compression = 0;
std::shared_ptr<DPDKManager> BytePSGlobal::_dpdk_manager;
std::mutex BytePSGlobal::_DPDKKey_mutex;
uint64_t BytePSGlobal::_DPDKkey = 0;
std::thread* BytePSGlobal::DPDKListener;
std::thread* BytePSGlobal::DPDKReset;
std::shared_ptr<TensorTableEntry> BytePSGlobal::DPDKTask[DPDK_KEY_TOTAL];
...
#ifdef USE_INCA
void BytePSGlobal::CreateDPDK(uint32_t host, int num_worker, int appID, int num_PS, int argc, int use_compression){
  sleep(1);
#ifdef SET_AFFINITY
  ...
  if(sched_setaffinity(0, sizeof(cpu_set), &cpu_set) < 0){
    printf("WARNING: CreateDPDK failed to set affinity\n");
  }
#endif
#ifdef SW_PS
#ifdef USE_TIMI
  char* argv[argc] = {"./client", "0", "1", "0", "1", "-a", "21:00.1", "--lcores=0@9,1@10,2@11,3@11", "--main-lcore=0", "--legacy-mem", "--socket-mem=10240"};
  // char* argv[argc] = {"./client", "0", "1", "0", "1", "-a", "21:00.1", "--lcores=0@16,1@17,2@18,3@18", "--main-lcore=0", "--legacy-mem"};
#else
  char* argv[argc] = {"./client", "0", "1", "0", "1", "-a", "c1:00.0", "--lcores=0@16,1@17,2@18,3@18", "--main-lcore=0", "--legacy-mem", "--socket-mem=1,10240"};
#endif // USE_[WORKER] 
  // char* argv[argc] = {"./client", "0", "1", "0", "1", "-a", "21:00.0", "-l", "0-1", "-m", "1024"};
#else
#ifdef USE_TIMI
  char* argv[argc] = {"./client", "0", "1", "0", "1", "-a", "21:00.1", "--lcores=0@9,1@10,2@11,3@11", "--main-lcore=0", "--legacy-mem", "--socket-mem=10240"};
#else
...
#endif
#endif // SW_PS
  _dpdk_manager = std::shared_ptr<DPDKManager>(new DPDKManager(host, num_worker, appID,
   num_PS, use_compression, argc, argv));
  if(!_dpdk_manager) printf("ERROR: Failed to create DPDK communication manager!\n");
  else printf("initialized dpdk_manager \n");
}
#endif
...
void BytePSGlobal::Init() {
  ...
  printf("Actual partition bytes after rounding: %d\n", _partition_bytes);
  ...
  // CPU_SET(17, &cpu_set);
  // if(sched_setaffinity(0, sizeof(cpu_set), &cpu_set) < 0){
  //   printf("WARNING: Global failed to set affinity\n");
  // }
  // // TODO: the following argument should be read from the command line in the future
  _use_compression =
      getenv("USE_COMPRESSION") ? atoi(getenv("USE_COMPRESSION")) : _use_compression;
  _use_compression =
      getenv("USE_TOPK") ? atoi(getenv("USE_TOPK")) : _use_compression;
  ...
  // int argc = 11;
  int argc = 10;
  std::thread _dpdk_main(BytePSGlobal::CreateDPDK, host, num_worker, appID, num_PS, argc, _use_compression);
  _dpdk_main.detach();
...
#ifdef USE_INCA
/* Clean up for DPDK manager */
  BytePSGlobal::DPDKListener->join();
  BytePSGlobal::DPDKReset->join();
  BytePSGlobal::DPDKListener = NULL;
  BytePSGlobal::DPDKReset = NULL;
