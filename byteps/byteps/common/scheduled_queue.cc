  ...
  switch (_qt) {
    ...
    case CONTEXT_PUSH:
      if (BytePSGlobal::IsRootDevice()) {
        _rt = BytePSGlobal::GetContextPushTable();
      }
      break;
    case COMPRESS:
    case PUSH:
...
std::shared_ptr<TensorTableEntry> BytePSScheduledQueue::getTask() {
  ...
  for (auto it = _sq.begin(); it != _sq.end(); ++it) {
    ...
    if (_rt) {
      ...
      /* Minghao: instead of context_push, for norm we can directly included in push? 
       * This also means that instead of updating scale and norm for _qt == COPYD2H, we should
       * actually do it for _qt == COPYH2D
      */
      // change task field here based on table field
      if (_qt == COPYD2H){
        (*it)->scale = _rt->GetKeyScale((*it)->key);
        // (*it)->norm = _rt->GetKeyNorm((*it)->key);
      }
      ...
      if (_qt == PUSH){
        (*it)->norm = std::max((*it)->norm, _rt->GetKeyNorm((*it)->key));
      }
      if (_qt == COPYH2D){
        // (*it)->scale = _rt->GetKeyScale((*it)->key);
        (*it)->norm = _rt->GetKeyNorm((*it)->key);
