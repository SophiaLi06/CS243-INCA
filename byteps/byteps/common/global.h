...
#ifdef USE_INCA
#include "dpdk/client/dpdk_manager.h"
#endif // USE_INCA
#ifdef USE_P4ML
// For P4ML
...
class BytePSGlobal {
 public:
  ...
  static uint32_t GetPartitionBound() {
    return _partition_bytes;
  }
...
#ifdef USE_INCA
  static int UseCompression() { return _use_compression; }
  static std::shared_ptr<DPDKManager> GetDPDK() { return _dpdk_manager; }
  static uint64_t GetDPDKKey(){
    std::lock_guard<std::mutex> lock(_DPDKKey_mutex);
    return _DPDKkey++;
  }
  static void CreateDPDK(uint32_t host, int num_worker, int appID, int num_PS, int argc, int use_compression);
  static std::thread* DPDKListener;
  static std::thread* DPDKReset;
  static std::shared_ptr<TensorTableEntry> DPDKTask[DPDK_KEY_TOTAL];
...
#ifdef USE_INCA
  static int _use_compression;
  static std::shared_ptr<DPDKManager> _dpdk_manager;
  static std::mutex _DPDKKey_mutex;
  static uint64_t _DPDKkey;
#endif // USE_INCA
#ifdef USE_P4ML
  /* For P4ML */
